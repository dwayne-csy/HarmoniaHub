...
export default function ReviewList() {
    ...
    try {
      await Promise.all(
        selectedIds.map(({ productId, reviewId }) =>
          axios.delete(`${BASE_URL}/admin/reviews/${productId}/${reviewId}`, {
            headers: { Authorization: `Bearer ${token}` },
          })
        )
      );
  ...
  const options = {
    ...
    elevation: 0,
    selectableRowsHideCheckboxes: false, // keep checkboxes visible
    customToolbarSelect: () => null, // hide default MUI delete icon toolbar
  };
